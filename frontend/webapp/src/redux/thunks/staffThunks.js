...
export const createNewStaff = (newStaff) => async (dispatch, getState) => {
  ...
  return axios
    .post(`/api/staff?HotelID=${HotelID}`, { ...newStaff, HotelID })
    .then((response) => {
    ...
    .catch((err) => {
      const errMessage =
        err.response.status === 500
          ...
          : err.response.data.message;
      dispatch(
        batchActions([loadStaffFail(), snackBarFail(errMessage), hideLoading()])
      );
    });
};
...
export const updateStaff = (updatedObj) => async (dispatch, getState) => {
  ...
  dispatch(showLoading());
  return axios
    .put(`/api/staff?HotelID=${HotelID}`, updatedObj)
    .then((response) => {
    ...
    .catch((err) => {
      dispatch(
        batchActions([snackBarFail(err.response.data.message), hideLoading()])
      );
    });
};
...
export const deleteStaff = (username) => async (dispatch, getState) => {
  ...
  dispatch(showLoading());
  return axios
    .delete(`/api/staff?HotelID=${HotelID}&username=${username}`)
    .then(() => {
      for (let i = 0; i < currentStaff.length; i++) {
